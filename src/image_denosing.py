...
def image_denoising(img):
    ...
    background=gaussian(img,60)
    img_prepared=denoised-background
    return denoised,background,img_prepared

def run_image_denosing(images):
    
    nframes,nrows,ncols=images.shape
    denoised=np.zeros_like(images)
    background=np.zeros_like(images)
    img_prepared=np.zeros_like(images)
    
    for frame in range(nframes):
        print('frame={} /t from total_frames={}'.format(frame,nframes))
        denoised[frame],background[frame],img_prepared[frame]=image_denoising(images[frame])
